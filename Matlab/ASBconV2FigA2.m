    ...
    while ((cc<1) && (b<MM))
        ...
        for t=1:Block
            ...
            for n=1:N                
                ...
                %    vi(t,n)=0-q(t);
                %end
                if q(t)*F<=g(n)%(q(t)+vi(t,n))*F<=g(t,n) 
                    vw(t)=vw(t)+1;
                    gbar(t,n)=g(n);
                end
            end
...
hold on
box on
plot(mv,'Color','k','LineStyle','-')
plot (mvbl, 'Color','k','LineStyle','--')
title('Expected violations in the dynamic mode, constant utility of violation opportunities')
xlabel('Deterrence resources (R)')
ylabel('Violations')
legend('E(v|R) for g^{t}_i=g_{i}','E(v|R) for the baseline, Fig 3.5)')
legend boxoff
ylim([0 100])
xlim([0 100])
hold off
