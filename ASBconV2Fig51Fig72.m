...
%figure
%hold on
%plot((lam-1)/PR,CS*100,'Color','k')
%plot((lam-1)/PR,CS2*100,'Color','k','LineStyle','--')
%ylabel('% cost saving','FontSize',30)
%xlabel('$\frac{({\lambda}-1)}{\rho}$','Interpreter','latex','FontSize',30)
%xlim([-5 65])
%ytickformat('percentage')
%legend('R^{*}({\rho},{\lambda}-1)','R^{*}(2*{\rho},2*{\lambda})', 'Location','East','FontSize',16)
%legend boxoff
%hold off
...
%xlim([0 25])
%ylim([-5 60])
%legend('R^{*}({\rho},{\lambda}-1)','R^{*}(2{\cdot}{\rho},2{\cdot}({\lambda}-1))', 'Location','East','Fontsize',20)
%legend boxoff
%hold off
